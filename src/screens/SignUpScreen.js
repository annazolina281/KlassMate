...
export default function SignUpScreen({ navigation }) {
  ...
  const validate = () => {
    ...
    else if (form.password.length < 8) newErrors.password = 'Must contain 8 characters';
    if (form.password !== form.confirmPassword) newErrors.confirmPassword = 'Must match both password';
    setErrors(newErrors);
  ...
  const Field = ({ icon, placeholder, fieldKey, secure, showToggle, onToggle, keyboardType }) => (
    <View>
      <View style={[styles.inputWrapper, errors[fieldKey] && styles.inputWrapperError]}>
...
const styles = StyleSheet.create({
  ...
  title: {
    fontSize: 26, fontWeight: '800', color: COLORS.primary, marginBottom: 8,
  },
